...
- `mutate` -> `dplyr::mutate_`
- `summari[sz]e` -> `dplyr::summari[sz]e_`
- `arrange(df, ...)` -> `dplyr::arrange_(.data, ...)`
...
For practical use, the following seems to work reasonably well
...
rename <- function(x, replace, warn_missing = TRUE) {
  ...
  .Deprecated("dplyr::rename_", "dplyr")
  do.call(dplyr::rename_, c(list(.data = x), as.list(setNames(names(replace), unlist(replace)))))
}
```
...
mtcars %>% count("gear")
mtcars %>% rename(list(mpg = "miles_per_gallon")) %>% head
```
